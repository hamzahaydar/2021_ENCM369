...
static u8 Counter=0x00; // counter that sets DAC1DATL to a sin value
u8 time = 0x00; // used to start increment or decrement of DAC1DATL
...
{ 
    T0CON0 = 0x90; // Enable TMR0, set as 16-bit timer, 1:1 post-scaler
    T0CON1 = 0x44; // set CLK source to Fosc/4 ,set to ASYNCH mode, set pre-scaler to 1:8
    TMR0H = 0x00;  // initialize MSB bits of timer 
    TMR0L = 0x00;  // initialize LSB bits of timer
...
{    
        if(time == 0x00)
        {
           DAC1DATL = UserAPP_au8sinTable[Counter+=4]; // Steps through DAC incrementing by 4 for 64 steps
        }   
        else
        {
            DAC1DATL = UserAPP_au8sinTable[Counter-=4]; // Steps through DAC decrementing by 4 for 64 steps
        }
        if(Counter == 255) //if DAC1DATL has reached the max sinusoid value begin decrement 
        {
            time = 0x01; // to turn on increment
        }
        if(Counter == 0x00) //if DAC1DATL has reached the min sinusoid value begin increment
        {
            time = 0x00; // to turn on decrement
        }
   ...
   void TimeXus(u16 User_Input)
   {
    ...
    u16 timer_end = 0xFFFF - User_Input; // configure timer to user specfication
    
    TMR0L = (u8) (timer_end & 0xFF); // Pre-sets TMR0L registers to the correct value
    TMR0H = (u8) ((timer_end >> 8)&0xFF); // Pre-sets TMR0H registers to the correct value
